...
class Scope:
    ...
    def err_code(self):
        self.resource.write_raw(':SYSTem:ERRor?')
    ...
    def set_wfm_source(self, ch_num = 1, source_str = None):
        ...
        if source_str:
            self.resource.write_raw(':WAVeform:SOURce {}'.format(source_str))
        else:
            self.resource.write_raw(':WAVeform:SOURce CHAN{}'.format(ch_num))
    ...
    def macro_setup(self,
        ...
        self.err_corr()
        self.resource.write_raw(':SYSTem:HEADer 1')
        remaining = {1,2,3,4}
        for number in channels_displayed:
            remaining -= set((number,))
            self.resource.write_raw(':CHANnel{}:DISPlay 1'.format(number))
            stdout.write(str(self.query_string(':CHANnel{}:DISPlay?'.format(number))+b'\n'))
        for number in remaining:
             self.resource.write_raw(':CHANnel{}:DISPlay 0'.format(number))
             stdout.write(str(self.query_string(':CHANnel{}:DISPlay?'.format(number))+b'\n'))
            
        for key in channels_coupling.keys():
            self.resource.write_raw(':CHANnel{}:INPUT '.format(key)+ channels_coupling[key])
            stdout.write(str(self.query_string(':CHANnel{}:INPUT?'.format(key))+b'\n'))
            
        if 64000 > average_count > 0:
            self.resource.write_raw(':ACQuire:AVERage:COUNt {}'.format(average_count))
            self.resource.write_raw(':ACQuire:AVERage 1')
        elif average_count == 0:
            self.resource.write_raw(':ACQuire:AVERage 0')
        else:
        ...
        if trace_points < self.point_lims[0]:
            self.resource.write_raw(':ACQuire:POINts:ANALog {}'.format(self.point_lims[0]))
        elif trace_points > self.point_lims[1]:
            self.resource.write_raw(':ACQuire:POINts:ANALog {}'.format(self.point_lims[1]))
        else:
            self.resource.write_raw(':ACQuire:POINts:ANALog {}'.format(trace_points))
        stdout.write(str(self.query_string(':ACQuire:POINts:ANALog?')+b'\n'))
        
        if sampling_rate < self.srate_lims[0]:
            self.resource.write_raw(':ACQuire:SRATe:ANALog {}'.format(self.srate_lims[0]))
        elif sampling_rate > self.srate_lims[1]:
            self.resource.write_raw(':ACQuire:SRATe:ANALog {}'.format(self.srate_lims[1]))
        else:
            self.resource.write_raw(':ACQuire:SRATe:ANALog {}'.format(sampling_rate))
        stdout.write(str(self.query_string(':ACQuire:SRATe:ANALog?')+b'\n'))
        
        self.resource.write_raw(':ACQuire:MODE '+mode)
        stdout.write(str(self.query_string(':ACQuire:MODE?')+b'\n'))
        
        self.resource.write_raw('TRIGger:SWEep '+trigger)
        stdout.write(str(self.query_string('TRIGger:SWEep?')+b'\n'))
        
        self.resource.write_raw(':WAVeform:FORMat ' + wave_format)
        stdout.write(str(self.query_string(':WAVeform:FORMat?')+b'\n'))
        
        self.resource.write_raw(':WAVeform:BYTeorder ' + wave_byteorder)
        stdout.write(str(self.query_string(':WAVeform:BYTeorder?')+b'\n'))
          
        self.resource.write_raw(':WAVeform:VIEW {}'.format(wave_view))
        stdout.write(str(self.query_string(':WAVeform:VIEW?')+b'\n'))
        
        self.resource.write_raw(':WAVeform:STReaming ' + streaming)
        stdout.write(str(self.query_string(':WAVeform:STReaming?')+b'\n'))
        
        self.resource.write_raw(':SYSTem:HEADer ' + header)
        stdout.write(str(self.query_string(':SYSTem:HEADer?')+b'\n'))
           
    def vertical_autoscale(self, ch_num):
        self.err_corr()
        self.resource.write_raw(':AUToscale:VERTical CHANnel{}'.format(ch_num))
    ...
    def acquire(self, timeout = np.inf, sleep_step = 0.01):
        ...
        """
        self.err_corr()
        self.resource.write_raw(':SINGle')
        i = 0
        t0 = time.time()
