...
#        self.device=rm.open_resource('TCPIP0::WINDOWS-E76DLEM::hislip0,4880::INSTR')
        ...
        self.device.write("SYSTem:HEADer 0")
...
#        time_range in seconds
    ...
    def query_wave_fast(self):
        ...
        """
        self.device.write_raw(":WAVeform:PREamble?")
        preamble = self.device.read_raw()
        ...
        self.device.write_raw(':WAVeform:DATA?')
        y_data = np.frombuffer(self.device.read_raw()[2:-1],dtype = np.dtype('>i2'))*origins[2]+origins[3]
